...
TEST_CASE("Counting and Finding") {
  ...
  SECTION("finding") {
    auto res =
        find_if(begin(m), end(m), [](const auto e) { return e.second > 10; });
    CHECK(res->first == 2);
  }
  SECTION("sort") {
    ...
    sort(begin(v2), end(v2),
         [](auto e1, auto e2) { return abs(e1) < abs(e2); });
    CHECK(v2[0] == 0);
    for (auto e : v1)
      cout << e << " ";
    cout << endl;
