...
TEST_CASE("modern cpp") {
    ...
    SECTION("test variadic") {
        ...
        [p2 = move(ptr)]() { cout << "move(steal) unique pointer into lambda " << p2[9] << endl; }();
    }SECTION("generic_lambdas") {
        vector lines{"cpp is awesome"s, "real developer's language"s, "let's do cool stuff"s, "brilliant a"s};
        sort(begin(lines), end(lines), [](auto const &a, auto const &b) { return a.length() < b.length(); });
        for (const auto &l: lines) cout << l << endl;
        ...
        for (const auto &[fn, ln] : mydict) cout << fn << " " << ln << endl;
